...
一跳 JOIN 投影、关联排序、`COALESCE`/null 排序和稳定分页的待扩展方案见 [Relation Query JOIN_LIST 投影与排序方案](../../../../docs/evolution/roadmap/crud/relation-query-join-list.md)。
...
- 需要“主表列表 + 一跳维表投影 + 跨表稳定排序”时，当前仍走定制 `Query*SceneHandler`；通用化方案见 [Relation Query JOIN_LIST 投影与排序方案](../../../../docs/evolution/roadmap/crud/relation-query-join-list.md)。
