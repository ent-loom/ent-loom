...
HTTP 合同不接收 `EXISTS` 关联过滤。需要按关联记录筛选根实体时，在 Query Scene Handler 内固定关系与过滤规则，再委托默认引擎：

```java
@Override
public List<Order> handle(
    QuerySpec<Order> spec,
    SceneDelegate<QuerySpec<Order>, List<Order>> delegate
) {
...
首期仅支持本地库、根实体一跳关系和目标实体直接字段；不要从 HTTP 请求直接透传关系名或过滤条件。
