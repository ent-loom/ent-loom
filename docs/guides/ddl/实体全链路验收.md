...
MySQL 8 验收使用专用 profile。目标实例默认是 `jdbc:mysql://127.0.0.1:3307/`；账号须具备建库、建表和删库权限。密码可用 Maven 属性或环境变量 `ENTLOOM_E5_MYSQL_PASSWORD` 提供，避免写入项目文件。

```bash
ENTLOOM_E5_MYSQL_PASSWORD='你的测试密码' \
JAVA_HOME=/Users/zubin/Library/Java/JavaVirtualMachines/azul-21.0.10/Contents/Home \
  ./mvnw -Pmysql-integration \
  -Dentloom.e5.mysql.url=jdbc:mysql://127.0.0.1:3307/ \
  -Dentloom.e5.mysql.username=root \
