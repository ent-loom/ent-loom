...
| MySQL DDL | `@EntDdlEntity`、`@EntDdlField`、`@EntDdlIndex` | 创建 `customer_profile`、主键自增和 `uk_customer_profile_display_name` 唯一索引 |
| CRUD | `@EntCrudEntity` | 生成实体 Runtime Model，并以 JDBC 在 H2 中执行最小 CRUD |
| DOC | `@EntDocEntity`、`@EntDocField` | 输出实体、字段、列名和字段类型文档模型 |

同一唯一索引同时存在 `@EntIndex(fields = {"displayName"})` 和 `@EntDdlIndex(fields = {"display_name"})`。前者保留 Meta 的 Java 字段身份，后者明确 DDL 使用的物理列名；两者的名称和唯一性必须保持等价。
