...
> 状态：In Progress
> 最近核验：2026-08-21
> 当前大项：C5，收窄 annotations 依赖
> 当前小项：C5，收窄 annotations 依赖
> 阻塞项：无
> 验收证据：C1-C4 已完成。C4：确认 `ent-loom-crud-core` 保持单一完整运行时 artifact；POM 直接编译依赖合同和 Core 架构守卫通过，JDK 21 下 Core 回归 198 项通过。当前进入 C5。
