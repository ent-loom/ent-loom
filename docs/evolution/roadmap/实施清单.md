...
> 当前小项：M3，CRUD 消费闭环
> 阻塞项：无
> 验收证据：C1-C7、M1、M2 已完成。M2：MetaConvention、日期时间内置约定、属性级来源追踪、显式 Meta Annotation 覆盖和 Starter Convention Bean 收集已落地；JDK 21 下 Meta Core `MetaConventionTest` 3 项、Meta Starter `EntLoomMetaAutoConfigurationTest` 7 项通过。当前进入 M3。
