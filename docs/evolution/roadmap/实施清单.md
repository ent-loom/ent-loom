...
> 当前小项：无
> 阻塞项：无
> 验收证据：C1-C7、M1、M2、M3、M4 已完成。M4：CRUD Core POM 依赖白名单和禁止 Meta Core/叶子实现依赖守卫、Convention 反向注册顺序稳定性与来源追踪测试已落地；最近补强提交为 `fe92384`（结构化值稳定裁决顺序）和 `5fca7e9`（`SourcedValue.ruleId` 相等语义兼容）。JDK 21 下完整 Maven Reactor `./mvnw test` 32 个模块全部 BUILD SUCCESS。
