...
> 阻塞项：无
> 验收证据：CRUD C1-C7、Meta M1-M4 已完成；DDL E1 Core 建表闭环已完成，Core 17 项、扩展 DDL Reactor 22 项测试通过。E2 进入待启动状态，既有 CRUD / Meta 验收证据保持不变。
...
- [ ] E2：实体发现与 Spring Boot 实际执行。当前入口：发现合同、消费者边界和真实执行准备。
- [ ] E3：字段与索引差异更新。
- [ ] E4：Meta -> DDL Adapter。
- [ ] E5：DDL、CRUD、DOC、UI 实体全链路验收。
...
- [ ] E2.1：发现合同覆盖显式类、包扫描、空输入、重复类和稳定顺序。
- [ ] E2.2：Spring 配置与 `enabled=false` 行为可验证，缺少真实 SPI 时有明确诊断。
- [ ] E2.3：Spring JDBC 执行器保留生成 / 执行 / 异常结果，并验证资源释放。
- [ ] E2.4：公开 API 消费者冒烟测试可编译并完成最小实体调用。
- [ ] E2.5：使用专用 profile 或 Testcontainers 完成一次 MySQL 8 建库 / 建表验证。
