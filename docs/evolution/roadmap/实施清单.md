...
> 当前小项：C1-2，Starter 配置 Key、默认 Bean 和 AutoConfiguration 条件回归
> 阻塞项：无
> 验收证据：C1-1 已完成；C1-2 已完成：`CrudStarterConfigurationContractTest` 4 项、定向回归 16 项、Starter 全量回归 82 项均通过（JDK 21；命令：`mvn -pl ent-loom-modules/ent-loom-crud/ent-loom-crud-spring-boot-starter -am -DskipTests=false test`）
