...
> 阻塞项：无
> 验收证据：CRUD C1-C7、Meta M1-M4 已完成；DDL 当前已建立 E1-E5 分阶段实施清单，尚未完成 E1 验收。既有 CRUD / Meta 验收证据和最近回归提交保持不变。
...
- [ ] E1：DDL Core 建表闭环。当前小项：E1.1 稳定建表模型与 SQL 生成合同。
- [ ] E2：实体发现与 Spring Boot 实际执行。
- [ ] E3：字段与索引差异更新。
- [ ] E4：Meta -> DDL Adapter。
...
- [ ] E1.1：元数据必填项、非法输入、主键 / 索引 / 默认值语义明确。
- [ ] E1.2：SQL 生成顺序稳定，Core 执行结果和异常合同明确。
- [ ] E1.3：DDL Core 单测通过，Core 无 Spring / Starter 依赖。
- [ ] 阶段收尾：记录测试命令和证据，更新 Architecture 当前事实后再进入 E2。
