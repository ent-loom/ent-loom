...
> 最近核验：2026-09-01
> 当前大项：无（最近完成 F1-F3：版本事实收敛与全链路验收）
> 当前小项：无
> 阻塞项：无
...
2. 当前一次只推进一个有明确验收边界的主线；完成后先收敛当前事实，再决定下一条主线。
...
- [x] F1：以代码、测试和组件 Architecture 为准盘点事实，识别 README、Meta 路线图和 Architecture 中的过期表述。
- [x] F2：统一公共入口；确认 Meta -> CRUD / DOC / DDL 静态 Adapter 已完成，UI Adapter 仍未建立。
- [x] F3：复用 E5 的 `CustomerProfile` 静态、MySQL 8 DDL、H2 + MockMvc 验收，不新增第二套跨模块测试。

详细范围、事实来源和完成门禁见 [版本事实收敛与全链路验收实施计划](版本事实收敛与全链路验收实施计划.md)。
