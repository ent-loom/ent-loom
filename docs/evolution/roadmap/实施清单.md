...
> 状态：In Progress
> 最近核验：2026-08-21
> 当前大项：C1，CRUD 外部契约回归门禁
> 当前小项：C1-1，增加成功与失败 JSON snapshot（已完成）
> 阻塞项：无
> 验收证据：`CrudHttpContractSnapshotTest`、`CrudHttpErrorMapperContractTest`；JDK 21 下执行 `mvn -pl ent-loom-modules/ent-loom-crud/ent-loom-crud-spring-boot-starter -am -Dsurefire.failIfNoSpecifiedTests=false -Dtest=CrudHttpContractSnapshotTest,CrudHttpErrorMapperContractTest test`
...
- [ ] C1：外部契约回归门禁。当前小项从 [CRUD 重构路线](crud/CRUD重构路线.md) 的“外部契约回归门禁”中选取。
