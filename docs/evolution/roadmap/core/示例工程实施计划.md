...
> 状态：进行中（单表示例 CI 验收作业和公开构件消费已确认，快速开始与最小商城已落地；整体 CI 仍有 Runtime Adapter 独立作业失败）
...
- 已确认：GitHub Actions 运行 `34198005492` 的“单表示例 MySQL 验收”作业通过；Maven Central `ent-loom 1.0.0` 在全新隔离 Maven 仓库中可被单表示例独立构建消费；正式快速开始已接入 Docusaurus 使用指南和侧边栏。
- 已完成：`mini-commerce-spring-boot` 的商品、客户、事务下单 Handler、订单详情、失败响应和独立验收脚本；脚本生命周期单测和本机真实 MySQL 验收均已通过，CI 已加入同一路径的独立验收作业。
- 范围说明：Maven Central `1.0.0` 本次确认的是无本机构件依赖的独立 Maven 构建；该已发布版本的真实启动仍受旧版 CRUD Starter 自动配置顺序影响，因此真实 HTTP/MySQL 门禁统一消费当前工作区安装到隔离仓库的构件。
- 当前风险：同一 GitHub Actions 运行的 `Runtime Adapter 测试` 作业失败，因此工作流整体结论为 failure；该失败不影响目标“单表示例 MySQL 验收”作业的实际成功结果，需单独排查。
