...
> 状态：进行中（单表与商城公开版本验收已通过；商城结构、权限及事务验收已补齐，待 GitHub 门禁确认）
...
- 已确认：GitHub Actions 运行 `34202738818` 的单表示例和商城 MySQL 验收作业、Maven Central 独立构建、Maven 测试和 Docusaurus 构建均通过；快速开始预览已接入 Docusaurus 使用指南和侧边栏。公开构件独立构建通过不代表公开版本真实启动验收通过。
...
- 发布状态：已确认 Maven Central 可获取 `ent-runtime 0.1.1` 和 `ent-loom 1.0.1`。CI 的公开消费者作业已从仅打包改为真实 HTTP/MySQL 验收；调整后的 GitHub 作业和 Runtime Adapter 门禁仍待确认。
...
- `PlaceOrderHandler` 通过公开 CRUD API 或项目自身的持久化端口读取主数据、写入订单；不得调用框架内部实现、复制 CRUD Controller，或绕过主体、权限和实体暴露治理。商品失效和不存在的客户/商品必须各有可观察的失败响应。
...
6. [ ] 确认 Runtime Adapter 的 GitHub 门禁恢复通过，统一计划、示例 README 和快速开始的版本与交付状态，再将本计划标记为完成。
