...
> 状态：进行中（单表示例本地开发接入完成，自动验收与 CI 已接入，待 CI 实跑确认）
...
- 待确认：CI 环境实际运行结果、公开 Maven 仓库构件消费验收与正式快速开始入口。
- 未开始：最小商城示例及其独立验收。

本机干净环境验收目前受 Docker Hub 的 `mysql:8.4` 镜像拉取连接拒绝阻断；已验证该失败路径会保存日志并清理验收资源，不能据此标记真实 MySQL 8.4 验收通过。
