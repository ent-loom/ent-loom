...
> 状态：进行中（示例和本地隔离发布验证已完成；待 `ent-runtime 0.1.1`、`ent-loom 1.0.1` 进入 Maven Central 后复验公开构件快速开始）
...
- 已确认：GitHub Actions 运行 `34202738818` 的单表示例和商城 MySQL 验收作业、Maven Central 独立构建、Maven 测试和 Docusaurus 构建均通过；正式快速开始已接入 Docusaurus 使用指南和侧边栏。
- 已完成：`mini-commerce-spring-boot` 的商品、客户、事务下单 Handler、订单详情、失败响应和独立验收脚本；脚本生命周期单测和本机真实 MySQL 验收均已通过，CI 已加入同一路径的独立验收作业。
- 范围说明：`ent-loom 1.0.0` 只确认了无本机构件依赖的独立 Maven 构建，真实启动受旧版 Starter 自动配置顺序影响；当前源码已修复并准备发布 `1.0.1`，本地隔离仓库的真实启动和 HTTP/MySQL 验收已通过。
- 当前风险：`ent-runtime 0.1.1` 和 `ent-loom 1.0.1` 尚未完成 Maven Central 发布；公开构件快速开始和 Runtime Adapter 的 GitHub 门禁需在发布后重新验证。
...
1. [x] 完整交付 `customer-profile-spring-boot`，包括 MySQL 8、本地启动、真实 `CREATE` 与 `DETAIL` 请求、SQL 状态核对及文档入口。
2. [x] 为单表示例建立可自动执行的构建、启动和 HTTP 冒烟验证，但不将其替代 `ent-loom-tests` 中的模块、消费者或 MySQL 专项测试；已确认 CI 作业通过并接入正式快速开始。
3. [x] 以单表示例已经稳定的公开依赖和配置方式为基线，实现 `mini-commerce-spring-boot` 的下单业务 Handler，并为其建立独立的干净环境验收。
4. [x] 商城示例作为业务协作的后续入口完成交付；已通过独立脚本生命周期测试和本机真实 MySQL 验收，并接入 CI 作业，文档中与单表示例并列引用。
5. [ ] 发布并验证 `ent-runtime 0.1.1` 与 `ent-loom 1.0.1`；确认 Maven Central 默认构件可启动示例、Runtime Adapter 作业恢复通过后，再将本计划标记为完成。
