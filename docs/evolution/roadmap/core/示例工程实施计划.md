...
- 代码组织：商城按 `customer/entity`、`product/entity`、`configuration`、`order` 分包；订单内部按 `controller`、`service`、`dto`、`model`、`repository`、`enums`、`exception`、`security` 组织。下单和查询均通过业务入口检查主体与订单动作权限，目录重构及生产主体适配后 JDK 21 单元测试 10 项、脚本生命周期测试 6 项和真实 MySQL/HTTP 验收均通过。
