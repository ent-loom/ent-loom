...
以 CRUD 导出结果文件为唯一场景，证明 `ent-loom` 可以通过可选 Adapter 使用 `ent-runtime` 的主体、任务和文件能力，同时保持两个核心仓库没有直接依赖。
...
R1-R5 已完成：当前闭环可用 runtime 内存存储验证主体、任务、流式文件、错误文件引用和 CRUD 下载守卫；这仍然是任务化合同验证，不是生产级 Worker。
...
- 不迁移 `PayloadCanonicalizer` 和 `StablePayloadCanonicalizer`。
...
| R5 | 导出纵向测试 | 已完成：主体、任务、文件结果和下载校验形成证据链 |

R1 与 R2 可以独立完成；R3-R5 必须串行收敛。每一阶段失败时只修正当前边界，不提前引入下一阶段基础设施。
...
只有 R1-R5 完成并证明重复实现造成实际维护成本后，才评估下一阶段：

1. 收敛 `PayloadCanonicalizer` 与幂等指纹计算。
