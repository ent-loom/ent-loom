...
> 状态：Partial（`EXISTS` 首期闭环已实现，`JOIN_LIST` 仍待实施）
> 当前事实：[Query 当前实现](../../../architecture/components/crud/查询.md)

当前默认查询使用 `ROOT_FIRST`：先按根实体过滤和分页，再批量补充关系数据。`EXISTS` 首期能力已仅通过
Java 内部受控模型开放：本地库、一跳关系、关联表 AND 条件、显式策略，且不展开关联行。关联排序和 JOIN
投影仍使用业务 Scene Handler。
...
当前 `EXISTS` 实现遵循该建议：关系路径与目标字段由 Runtime Model 绑定；关联表追加逻辑删除与治理范围
条件；data 与 count SQL 复用同一谓词。首期不支持多跳、跨服务、关联展开、投影、排序或 OR 条件组。
