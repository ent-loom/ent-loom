...
- [x] `JdbcWriteMissClassifier` 若继续保留，只能作为诊断或旧引擎兼容部件，不进入 DAO 的稳定业务合同。
- [x] 形成并发测试设计，证明 DAO 正确性不依赖“写入后再查询”的竞态分类；测试实现在 D2 完成。

验收：无需 DAO 自建事务或假设多条语句使用同一连接，也能给出稳定且不泄露数据存在性的写入结果。

- 验收日期：2026-09-17
- 核验方式：DAO 与默认 Engine 合同对照；D2 未命中及并发测试用例设计核验
- 验收结果：0 行异常只由 `expectedVersion` 是否存在决定，1 行为唯一正常结果，大于 1 行视为持久化不变量破坏；DAO 不执行写后分类查询
- 边界确认：`JdbcWriteMissClassifier` 暂留为默认 Engine 兼容部件，不进入 DAO 合同；本阶段不修改现有 Engine 运行行为
- 遗留事项：D2 实现并发与无写后查询测试；进入 D0.3 校正 insert 与行约束
