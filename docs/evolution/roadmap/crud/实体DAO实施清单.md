...
本文是[实体 DAO](../../../architecture/components/crud/实体DAO.md)的待执行清单。实施遵循“先完成最小安全闭环，再按真实需求演进”：先围绕一个真实样板实体完成按主键、单表、单数据源的安全读写闭环，再根据真实调用需求增加列表、分页、批量和分片能力。
...
- [x] 明确 `JdbcWriteMissClassifier` 不进入目标架构；默认 Engine 主键写入切换到 DAO 时同步删除分类器、调用点和精细分类测试。
...
- 边界确认：D0 只校正合同，不提前修改 JDBC；D3 切换默认 Engine 时直接删除 `JdbcWriteMissClassifier`，不保留双路径或开关
...
- [ ] 选定一个真实、无复杂关系的样板实体及其真实 Service 或 Gateway 调用入口，作为 D1-D4 贯穿式验收对象。
...
- [ ] 定义“目标不存在或不可写”“版本条件写入冲突”“非法范围”“非法写入字段”等稳定异常；版本条件写入冲突默认不可据此自动重试。
- [ ] 所有状态和类型优先使用带中文名称说明的枚举。
- [ ] 使用已选样板实体校验范围 AST、访问上下文和写入选项能够表达真实调用，不为样板实体增加专用分支。
...
- [ ] 已选样板实体完成 Factory -> DAO -> H2 的主键读写闭环，且使用与真实入口相同的元数据与范围模型。

## D3：以 DAO 重构 Gateway / Engine 主键路径
...
- [ ] 外部越权集成测试覆盖租户、组织和普通查询条件，证明真实入口只能收窄最终范围；可信 Service 显式范围与 Resolver 范围使用同一校验和绑定路径。
- [ ] 默认 Engine 的主键读写在一个闭环内全部切换到 DAO，不保留原主键 SQL 备用路径。
...
- [ ] 非空 scene、范围拒绝、逻辑删除和乐观锁符合目标合同。
- [ ] 审计、幂等和事务边界由目标架构测试完整覆盖。
