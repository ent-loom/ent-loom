...
4. 当前框架尚未上线，不承担历史 API、配置或运行行为兼容责任；发生合同调整时默认执行干净重构。
5. 干净重构必须在同一闭环内切换全部仓库内调用者并删除旧包、旧模型、旧配置、旧测试和重复实现，不设置废弃期，不增加适配层、兼容开关或新旧双入口。
6. 测试以目标合同为准，不为保留错误或过时行为编写新旧等价测试；确需保留的业务语义必须直接写入目标合同并由新路径验证。
7. 当前事实与目标类型必须在文档中明确区分。
