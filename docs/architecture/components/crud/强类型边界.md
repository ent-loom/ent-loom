...
本文定义动态 CRUD 入口与强类型业务代码之间的稳定边界。当前代码已经具备统一 Binder、稳定的
`UpdatePatch<T>`、默认不可变实现和强类型 Handler 基类；聚合 Patch 继续保留独立职责。
...
实体模式当前以业务侧专用 Adapter 落地。Adapter 只负责把查询对象、创建 DTO 和
`UpdatePatch<T>` 规范化为 `QuerySpec` / `CommandSpec`，然后调用 `QueryGateway` / `CommandGateway`；
它不拥有权限、数据范围、SQL 或审计实现。没有两个真实调用者时，不提炼 Core 公共 Facade。
...
| 普通 UPDATE | `command.patch.UpdatePatch<T>` | 已稳定 |
...
普通 UPDATE 已不再暴露临时 `command.patch.EntityPatch<T>`；聚合 `EntityPatch<T>` 仍只负责关系 Patch，不能替代普通局部更新 API。
...
- CREATE 的主键是否进入请求由 `EntityIdPolicy` 决定：`EXPLICIT` 通过 `WriteCommand.id` 传入，
  `GENERATED` 由数据库回填；主键不会作为普通可写字段进入 `values`。
...
当前实施顺序见 [CRUD 重构路线](../../../evolution/roadmap/crud/CRUD重构路线.md)，设计理由见 [强类型 Handler 决策](../../../evolution/decisions/crud/强类型命令处理器决策.md)。

U3 的 `CustomerProfileCrudAdapter` 已用 H2 + Spring Boot JDBC 验证：它与等价基础 Spec 的
CREATE、PAGE、UPDATE 共享主体、权限、数据范围、SQL 结果和审计字段；验证位于
`CustomerProfileCrudAdapterEquivalenceTest`。该 fixture 使用显式主键，数据库生成主键的映射
仍由专门的 Runtime Model / DDL 合同另行验证。
