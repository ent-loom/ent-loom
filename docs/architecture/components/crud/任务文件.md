...
- 默认实现面向小文件。
- `FileService` 已提供流式保存和 `openStream` 入口；内存实现仍会聚合内容，本地实现可直接打开本地文件流，不承诺所有后端都零拷贝。
- 对象存储、后台清理和任务 worker 由业务替换或后续扩展。

## 与 ent-runtime 的适配边界

可选的 `ent-loom-runtime-adapter` 将 CRUD 的 `PENDING` 映射为 runtime 的 `CREATED`，其余生命周期按同名终态流转。runtime 文件引用正式承接主体、用途和过期时间；Adapter 读取时按 `fileId` 重新获取真实引用，下载仍必须先经过本页的任务、主体、用途、过期和元数据守卫。

适配器不会把 CRUD 的 `EXPIRED` 任务状态写入 runtime；文件过期通过 runtime 文件异常和 CRUD `FILE_EXPIRED` 错误码表达。runtime 的文件未找到、过期、大小不一致和存储失败不会直接泄漏为消息字符串。
