...
flowchart TB
    ...
    sql["GuardedSqlExecutor"]
    miss["当前待删除<br/>JdbcWriteMissClassifier"]
...
当前 `JdbcWriteMissClassifier` 会在 update/delete 影响行数为 0 时做三次存在性检查：
...
这是待实体 DAO 主键写入闭环直接替换的当前事实，不是目标能力。存在性检查发生在主写入之后，期间目标状态可能被其他事务改变，因此分类结果不能作为“不存在、越权或版本冲突”的竞态安全证明。

默认 Engine 切换到 DAO 时，`JdbcWriteMissClassifier`、三次存在性查询、调用点和对应精细分类测试必须在同一闭环删除，不设置开关或备用路径。替换后的 0 行语义只由主写入是否携带 `expectedVersion` 决定，并直接按[实体 DAO](实体DAO.md#写入未命中语义)合同测试验收。
