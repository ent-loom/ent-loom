...
flowchart TB
    ...
    handler["JdbcEntityDaoCommandHandler"]
    dao["EntityDao<br/>单表实体"]
    fallback["JdbcCrudCommandHandler<br/>默认命令扩展能力"]
    ...
    handler --> dao --> sql
    handler -. "DAO 合同不支持" .-> fallback
    fallback --> writeCommand --> mapper
    fallback --> predicate --> sql
```
...
显式主键实体的单条、批量和 `SAVE_OR_UPDATE` 命令统一进入 `JdbcEntityDaoCommandHandler`。批量在 Handler 层逐项复用 DAO，事务仍由外层 Service / Gateway 编排；这不等同于新增 DAO 原生批量公共合同。数据库生成主键实体虽然已支持直接调用 EntityDao，当前默认 Command Handler 仍进入 `JdbcCrudCommandHandler` 回退路径。
...
flowchart LR
    ...
    jdbc["JdbcEntityDaoCommandHandler"]
...
`JdbcWriteMissClassifier` 及写入后的存在性分类查询已经删除。显式主键实体由 DAO 将 `0` 行统一转换为“目标不存在或不可写”；生成主键等回退路径采用相同异常，不再区分不存在、越权或逻辑删除，避免竞态分类和存在性泄露。
...
- 标识符白名单默认唯一来源为 `ResourceCatalogAdapter -> CrudRuntimeModel -> EntityMetaRegistry`，不维护独立字段或关系清单。
- 当前不提供细粒度附加白名单配置；框架集成方可替换 `EntityMetaRegistry` 或 `SqlIdentifierAllowlistValidator` Bean。
