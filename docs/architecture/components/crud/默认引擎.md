...
这是当前默认 Engine 的兼容行为，不是实体 DAO 的稳定业务合同。存在性检查发生在主写入之后，期间目标状态可能被其他事务改变，因此分类结果不能作为“不存在、越权或版本冲突”的竞态安全证明。实体 DAO 首期只依据主写入是否携带 `expectedVersion` 映射粗粒度异常，不调用该分类器；默认 Engine 迁移到 DAO 前继续保留现状，并由新旧路径等价测试管理行为变化。
