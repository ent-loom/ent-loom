...
flowchart TB
    ...
    dao["EntityDao<br/>显式主键实体"]
    fallback["JdbcCrudCommandHandler<br/>生成主键等扩展能力"]
...
显式主键实体的单条、批量和 `SAVE_OR_UPDATE` 命令统一进入 `JdbcEntityDaoCommandHandler`。批量在 Handler 层逐项复用 DAO，事务仍由外层 Service / Gateway 编排；这不等同于新增 DAO 原生批量公共合同。数据库生成主键等超出当前 DAO 合同的实体才进入 `JdbcCrudCommandHandler` 回退路径。
