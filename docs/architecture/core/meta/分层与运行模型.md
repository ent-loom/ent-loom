...
- 实体、字段、索引和关系等通用事实。
- 字段 label、description、字段角色等跨模块语义；CRUD 外部输入要求不属于 Meta。
...
- Meta -> DDL Adapter 生成 DDL Runtime Model，并保留 DDL 原生属性覆盖与诊断边界。
- Meta Starter 根据 classpath 和配置条件装配 CRUD/DOC Adapter；DDL Adapter 由 DDL 集成模块独立装配。
- UI 尚无正式 Meta Adapter；E5 仅验证公开 UI Contract 的静态投影。
