...
适配器测试覆盖主体往返、文件流和元数据、流大小错误、runtime 文件异常映射、任务状态和上下文、结果文件主体归属、导入无结果文件成功、导出缺结果文件拒绝、错误文件引用，以及使用内存 Store 的任务文件下载守卫。该测试手工构造 `CrudTask` 和文件引用，未经过 `ExportGateway -> QueryEngine -> ExportEngine`，因此不代表真实异步导出或生产 Worker 已完成。
