...
ThreadLocal 的进一步收窄见 [CRUD 重构路线](../../../evolution/roadmap/crud/CRUD重构路线.md)。

## 异步任务快照

Import/Export 的 `CrudTaskContextSnapshot` 会保存任务需要的 operation key、主体、已授予 scope、最终治理 scope 和审计上下文（`requestId`、`traceId`）。本地任务服务会持久化这些字段，重新加载后仍可用于显式重新授权。

Worker 必须显式读取任务快照并将其转换为新的 Spec 或显式上下文，不重新读取创建请求线程的 ThreadLocal。快照字段是副本，不能通过修改创建请求对象反向改变任务权限。

当前默认 Import/Export 仍以同步小文件闭环为主；完整异步 Worker 调度、快照过期和重新授权策略属于后续能力，不能描述为已经完成。
