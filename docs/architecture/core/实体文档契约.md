...
当前仓库的 `MetaDocAdapter.buildOne/buildAll` 仍是内部适配能力，`EntityDocCoreService` 返回的兼容 Map 还包含实现细节。本文和 [JSON Schema](https://github.com/ent-loom/ent-loom/blob/main/docs/contracts/entity-documentation-v1.schema.json) 是 D3 的目标公共合同；Meta Spring Boot Starter 已提供默认关闭、复用 CRUD 主体解析器并要求业务按主体提供暴露策略解析器的契约服务装配入口，并提供默认关闭的 `GET /api/ent-doc/contract` 只读入口。该入口仍需业务应用显式启用契约服务、HTTP 开关和主体策略解析器，当前尚未提供 Workbench 接口。
...
| `DocFieldModel` | `property`、`javaType`、`name`、`description`、`example(s)`、`inputRequired`、`readOnly`、长度、`fieldKind`、`role`、约束 | `property`、`type`、`label`、`description`、`examples`、约束、可安全表达的字段能力 | `hidden=true` 的字段省略；不输出 `column`、完整 Java 类型或 `createDefaultValue` |
...
| `inputRequired`、`nullable`、`readOnly` | 否 | 分别表示创建请求输入要求、空值能力和静态只读能力；未知时省略，不以 `false` 冒充已确认 |
...
当前已完成 D3 前置事实盘点、v1 契约草案、`ent-loom-doc-core` 内部的 `EntityDocumentationProjector` 安全字段投影、`MetaDocAdapter.buildDocumentationContract` 的显式策略门槛和默认关闭的只读 HTTP 入口。商城示例另已提供 `production` profile，将 Servlet 容器认证主体接入现有 `CrudSubjectResolver` SPI，并在真实 HTTP/MySQL 验收中验证匿名主体只能获得空目录，不能调用 CRUD 或订单入口。证据来自 `DocEntityModel`、`CrudRuntimeModel`、Meta Descriptor、现有 DOC Adapter 回归、投影器测试、Starter MockMvc 合同测试，以及 `customer-profile-spring-boot` 和 `mini-commerce-spring-boot` 两个示例的真实 HTTP/MySQL 验收。当前仍缺少：

- 真实业务应用接入 Spring Security、网关或容器认证提供方后的认证授权回归；
- 不同认证提供方的租户、组织和数据范围映射回归。
