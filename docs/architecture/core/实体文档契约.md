...
当前已完成 D3 前置事实盘点、v1 契约草案、`ent-loom-doc-core` 内部的 `EntityDocumentationProjector` 安全字段投影、`MetaDocAdapter.buildDocumentationContract` 的显式策略门槛和默认关闭的只读 HTTP 入口。证据来自 `DocEntityModel`、`CrudRuntimeModel`、Meta Descriptor、现有 DOC Adapter 回归、投影器测试、Starter MockMvc 合同测试，以及 `customer-profile-spring-boot` 和 `mini-commerce-spring-boot` 两个示例的真实 HTTP/MySQL 验收。当前仍缺少：
