...
诊断应包含实体、字段、属性、双方来源、规则标识和建议处理。结构性错误默认 fail-fast；普通覆盖冲突可以由统一策略选择 fail、warn 或 ignore。
...
详细依赖方向见 [组件边界与依赖规则](component-boundaries.md)。
...
- 通用 `Contribution / RuleId / Priority` 和属性级 Resolver。
- Meta 与 CRUD Project Convention 的完整注册和消费闭环。
- DDL、UI Adapter。
- 所有组件统一的冲突策略与来源诊断。

实施顺序只在 [Meta 当前实施计划](../../evolution/roadmap/meta/metadata-resolution-phase-plan.md) 维护。
