...
5. annotations 模块保持轻量，只依赖 CRUD API 和确有运行时类型需求的最小 Meta enums，不依赖 Meta contract、Starter 或执行引擎。
6. 业务项目可以提供 Convention、Handler 和治理实现，但不能反向成为框架 Core 的依赖。

CRUD Core、CRUD Starter 和 Meta Core 的架构守卫分别固定在各自模块测试中，并覆盖 Starter 主包归属、
Core 禁止反向依赖、Meta Core 独立运行和旧配置包不回流。CRUD Core 另有 Maven POM 白名单守卫，明确禁止
直接依赖 `ent-loom-meta-core`、JDBC Engine 和 Spring Boot Starter。
...
最近核验：JDK 21 下完整 Maven Reactor `./mvnw test` 通过。
