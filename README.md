...
- **业务表达意图**：优先使用实体、值对象、强类型 DTO 和业务动作，常规场景只声明业务意图与例外。
- **框架承接细节**：通过明确的默认约定统一处理类型适配、基础校验、错误文案和协议转换。例如，业务表达“必填”，框架负责映射相应类型的检查；业务不必为同一意图重复选择底层校验方式。
- **默认简洁、例外可扩展**：通过职责明确的策略、组合或扩展点承接特殊业务，避免为简单场景堆叠配置、继承层级或 Hook。
- **约定有边界**：默认行为应可覆盖、可诊断；不凭技术类型猜测业务规则，不隐藏局部更新与整体替换等语义差异。业务不变量和业务编排仍由业务实现。

以上是设计与演进准则，不表示相关能力均已实现；具体支持范围以当前契约和下方能力表为准。
